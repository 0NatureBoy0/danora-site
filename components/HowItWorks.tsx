const teacher = [
  ...
  { t: "ثبت آگهی", d: "دو آگهی اول رایگان است؛ بعد از آن پلن انتخاب کن." },
...
const student = [
  ...
  { t: "تأیید و پرداخت", d: "بعد از پرداخت کارمزد، آی‌دی‌ها رد و بدل می‌شود." },
];
      ...
      <ol className="mt-6 space-y-5">
        {steps.map((s, i) => (
          <li key={s.t} className="flex gap-4">
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-600 text-xs font-black text-white">
              {i + 1}
            </span>
            <div>
              <p className="font-bold">{s.t}</p>
              <p className="mt-1 text-sm leading-7 text-slate-500">{s.d}</p>
            </div>
          </li>
        ))}
      </ol>
